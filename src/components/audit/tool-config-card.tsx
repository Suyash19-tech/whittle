...
/**
 * ToolConfigCard — premium configuration card
 * Custom-styled select, elegant seat counter, fintech-grade inputs
 */
export function ToolConfigCard({
    ...
    const adjustSeats = (delta: number) => {
        const next = Math.max(1, seats + delta);
        onSeatsChange(next);
    };

    const adjustSpend = (delta: number) => {
        const next = Math.max(0, monthlySpend + delta);
        onSpendChange(next);
    };
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Configure your plan</p>
                    </div>
                </div>
                <button
                    onClick={onRemove}
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-7 pr-3 text-center text-sm font-semibold text-slate-900 transition-all hover:border-slate-300 focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-500/20"
                                min="0"
                            />
                        </div>
                        <button
