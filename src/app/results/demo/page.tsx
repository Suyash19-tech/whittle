...
import { ArrowLeft, ArrowRight, TrendingDown, AlertCircle, CheckCircle2, Sparkles, Download, Share2, ChevronRight, Layers, Zap, Check, Copy } from 'lucide-react';
...
export default function ResultsDemoPage() {
  ...
  const { summary, score, recommendations, insights, aiSummary } = storeResults ?? MOCK_AUDIT;
          ...
          <div className="flex h-14 items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onShare}
                className="flex h-8 cursor-pointer items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 transition-all hover:border-sky-300 hover:bg-sky-50 hover:text-sky-700"
              >
                {copied && shareUrl ? <Check className="h-3.5 w-3.5 text-teal-500" /> : <Share2 className="h-3.5 w-3.5" />}
                {copied && shareUrl ? 'Copied!' : 'Share'}
              </button>
              <button
                type="button"
                className="flex h-8 cursor-pointer items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 transition-all hover:border-slate-300 hover:bg-slate-50"
              >
          ...
          {shareUrl && (
            <div className="mb-6 flex items-center gap-3 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold text-teal-800">Share link ready</p>
                <p className="mt-0.5 truncate font-mono text-xs text-teal-700">{shareUrl}</p>
              </div>
              <button
                type="button"
                onClick={onCopyBanner}
                className="flex flex-shrink-0 items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-teal-700"
              >
            ...
            <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-900 to-slate-800 px-8 py-10 text-center">
              ...
              <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
                ...
                <button
                  type="button"
                  onClick={onShare}
                  className="inline-flex h-11 cursor-pointer items-center gap-2 rounded-xl border border-slate-700 bg-transparent px-6 text-sm font-semibold text-slate-300 transition-all hover:border-slate-500 hover:bg-slate-800"
                >
                  {copied && shareUrl ? <Check className="h-4 w-4 text-teal-400" /> : <Share2 className="h-4 w-4" />}
                  {copied && shareUrl ? 'Link copied!' : 'Share this report'}
                </button>
              </div>
            </div>
          </motion.div>
